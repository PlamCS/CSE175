...
# object of the RoadMap class encodes a graph containing locations and
# road segments. The class includes relevant methods for the objects.
#
# David Noelle - Tue Sep  5 14:52:32 PDT 2023
#
...
class RoadMap:
    ...
    def path_cost(self, start, road_list):
        ...
        if (start not in self.road_dict) or (road_list == {}):
            return 0.0
        else:
            path = start
            total = 0.0
            for road in road_list:
                if road not in self.road_dict[path]:
                    break
                cost = self.connection_dict[path][self.road_dict[path][road]]
                total += cost
                path = self.road_dict[path][road]
            return total